...
const EventForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setSaving(true);
        try {
            const tickets = form.tickets.map(t => ({ type: t.type, price: parseFloat(t.price), available: parseInt(t.available) }));
            const payload = {
                ...form,
                tickets,
                totalCapacity: tickets.reduce((sum, t) => sum + t.available, 0),
            };
            if (isEditing) {
                    ...
                    <div className="space-y-4">
                        {form.tickets.map((ticket, idx) => (
                            <div key={idx} className="flex items-end gap-3 p-4 bg-surface-50 rounded-xl">
                                ...
                                <div className="w-32">
                                    <Input label="Price (₹)" type="number" value={ticket.price} onChange={(e) => handleTicketChange(idx, 'price', e.target.value)} placeholder="99" icon={HiCurrencyRupee} />
                                </div>
                                <div className="w-28">
